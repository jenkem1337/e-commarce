...
try {
    ...
    if (getenv("APP_ENV") == "docker"){
        $envFile = Dotenv\Dotenv::createImmutable(__DIR__, [".env.docker"]);
    } else {
        $envFile = Dotenv\Dotenv::createImmutable(__DIR__, [".env"]);
    }
    $envFile->load();
    ...
    $f->registerRoute("GET", "/orders/user", new FindAllOrderWithItemsByUserUuidCommand($orderController));
